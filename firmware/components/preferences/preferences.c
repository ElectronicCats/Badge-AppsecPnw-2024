...
static const char* TAG_PREFERENCES = "preferences";
...
void _check_started() {
  if (!_started) {
    ESP_LOGE(TAG_PREFERENCES,
             "Preferences not started! Call preferences_begin() first!");
    abort();
  }
}
...
esp_err_t _commit() {
  if (_return_err) {
    ESP_LOGE(TAG_PREFERENCES, "Error (%s) writing value!",
  ...
  if (_return_err != ESP_OK) {
    ESP_LOGE(TAG_PREFERENCES, "Error (%s) committing value!",
             esp_err_to_name(_return_err));
    return _return_err;
  }
...
esp_err_t preferences_begin() {
  ...
  if (_return_err != ESP_OK) {
    ESP_LOGE(TAG_PREFERENCES, "Error (%s) initializing NVS!",
             esp_err_to_name(_return_err));
    return _return_err;
  }

  ESP_LOGI(TAG_PREFERENCES, "Opening Non-Volatile Storage (NVS) handle...");
  _started = true;
  _return_err = nvs_open("storage", _read_only ? NVS_READONLY : NVS_READWRITE,
                         &_nvs_handler);

  if (_return_err != ESP_OK) {
    ESP_LOGE(TAG_PREFERENCES, "Error (%s) opening NVS!",
             esp_err_to_name(_return_err));
    return _return_err;
  }
...
void preferences_end() {
  ESP_LOGI(TAG_PREFERENCES, "Closing Non-Volatile Storage (NVS) handle...");
  nvs_close(_nvs_handler);
  _started = false;
}

esp_err_t preferences_clear() {
  ESP_LOGI(TAG_PREFERENCES, "Clearing NVS...");
...
esp_err_t preferences_remove(const char* key) {
  ...
  if (_return_err != ESP_OK) {
    ESP_LOGE(TAG_PREFERENCES, "Error (%s) removing key!",
             esp_err_to_name(_return_err));
    return _return_err;
  }
  ...
  _return_err = nvs_commit(_nvs_handler);
  if (_return_err != ESP_OK) {
    ESP_LOGE(TAG_PREFERENCES, "Error (%s) committing key removal!",
             esp_err_to_name(_return_err));
    return _return_err;
  }
...
int8_t preferences_get_char(const char* key, int8_t default_value) {
  ...
  if (_return_err == ESP_ERR_NVS_NOT_FOUND) {
    ESP_LOGI(TAG_PREFERENCES, "The value is not initialized yet!");
...
esp_err_t preferences_get_string(const char* key,
  ...
  if (length > max_length) {
    ESP_LOGE(TAG_PREFERENCES, "The value is too long for the buffer!");
