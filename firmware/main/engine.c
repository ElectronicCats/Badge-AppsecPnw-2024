...
OWASPVulns vulns[] = {{"A01", "Broken Access Control"},
...
void create_owasp_commmand(uint8_t* dest_buffer, OWASPProfile* profile) {
  ...
  dest_buffer[1] =
      profile->vuln->cwe[0];  // Copiar el primer carácter (el número)
  for (int i = 1; i < sizeof(profile->vuln->cwe); i++) {
    dest_buffer[i + 1] =
        profile->vuln->cwe[i];  // Copiar el resto de los caracteres
  }
}

void create_owasp_commmand_identifier(uint8_t* dest_buffer,
                                      OWASPBaseAction* attack) {
  dest_buffer[0] = '2';  // Agregar el número 1 después del primer carácter
  dest_buffer[1] =
      attack->identifier[0];  // Copiar el primer carácter (el número)
  for (int i = 1; i < sizeof(attack->identifier); i++) {
    dest_buffer[i + 1] =
        attack->identifier[i];  // Copiar el resto de los caracteres
  }
}
